...
const DashboardCardModal: React.FC<DashboardCardModalProps> = ({
    ...
    if (node.type !== "Folder") {
      return (
        <div className={"dashboard-card-modal-file-container"}>
          <img
            className="svg-icon"
            src={getFileType(node.name.split(".").pop() as string)}
            alt={`File Type`}
          />
          <span>{node.name}</span>
        </div>
  ...
  async function getDriveItems(
    ...
    const response = await buildAxiosRequestWithHeaders(
      "GET",
      `/drive-items?email=${user.email}&provider=${connectionProviderTitle}`,
      headers,
      {}
    );
        ...
        <div className={"modal"} onClick={stopPropagation}>
          <div className={"dashboard-card-modal-grid"}>
            <div className={"dashboard-card-modal-drive-information"}>
              <img
                src={CONNECTION_LOGOS[connectionProvider]}
                alt={`Logo for ${connectionProvider}`}
              />
              <p>
                {t(
                  "main.dashboard.dashboardCardModal.driveInformation.accountDetails"
                )}
              </p>
              <span>{email}</span>
              <span>
                {t(
                  "main.dashboard.dashboardCardModal.driveInformation.usedStorage"
                )}{" "}
                {usedStorage > 0.0 ? usedStorage : "< 0"}/GB
              </span>
              <span>
                {t(
                  "main.dashboard.dashboardCardModal.driveInformation.totalStorage"
                )}{" "}
                {totalStorage}/GB
              </span>
              {breakdownData && (
